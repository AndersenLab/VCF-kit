...
if __name__ == '__main__':
    ...
    if args["--region"] is not None:
        ...
        for chrom, bp_range in region_check:
            ...
            within_lower = within_range(
                int(bp_range.split("-")[0]), 0, v.contigs[chrom]["length"])
            within_upper = within_range(
                int(bp_range.split("-")[1]), 0, v.contigs[chrom]["length"])
            if not within_lower or not within_upper:
                error("The range (%s) falls outside of CHROM length (%s)" %
                      (bp_range, v.contigs[chrom]["length"]))
    else:
        args["--region"] = ""
    ...
    if args["listvars"] == True:
        ...
        if args["<y>"] is None:
            ...
            query, analysis_dir, filename, variable = v.query(
                args["<x>"], region=args["--region"])

            if args["<x>"] == "POS":
                # Facet by Chromosome Automatically
                print("")
                print(
                    bc("Plotting Position; Automatically facetting by Chromosome", "BOLD"))
                print("")
        ...
        if variable["type"] == None:
            ...
            aes = "x=" + variable["df"] + ",y=tstv_ratio"
            ...
            aes = "x=" + \
                variable["df"] + ",y=tstv_ratio, fill=" + \
                variable["df"] + ",color=Sample"
            # Filter out sparse data.
            opts.filters += "df <- filter(df, nTransitions + nTransversions > 10)"
            opts.add += " + \n stat_smooth(aes(x={var}, y=tstv_ratio))".format(
                var=variable["df"])
            Rcode = get_plot("scatter").format(**locals())
            print("")
            print(
                bc("Plotting scatter: ts/tv ratio x " + variable["df"], "BOLD"))
            print("")

        elif variable["type"] in ["String"]:
            # Box Plot (categorical)
            aes = "x=" + variable["df"] + \
                ",y=tstv_ratio, fill=" + variable["df"]
            opts.add += " + \n geom_jitter(aes(" + aes + "))"
        ...
        query, filename = v.compare_vcf(
            variable=args["--x"], pairs=args["--pairs"])
        variable = v.resolve_variable(args["<x>"])
